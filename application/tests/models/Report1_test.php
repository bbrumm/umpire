...
class Report1_test extends TestCase {
    ...
    public function test_GetDataQuery() {
        ...
        $expected = "SELECT last_first_name, short_league_name, club_name, age_group, SUM(match_count) AS match_count FROM dw_mv_report_01 WHERE age_group IN ('Four') AND short_league_name IN ('Five','Six') AND region_name IN ('Seven') AND umpire_type IN ('First','Second','Third') AND season_year = 2018 GROUP BY last_first_name, short_league_name, club_name ORDER BY last_first_name, short_league_name, club_name;";
    ...
    public function test_GetReportColumnQuery() {
        ...
        $expected = "SELECT DISTINCT short_league_name, club_name FROM dw_mv_report_01 WHERE age_group IN ('Four') AND short_league_name IN ('Five','Six') AND region_name IN ('Seven') AND umpire_type IN ('First','Second','Third') ORDER BY short_league_name, club_name;";
