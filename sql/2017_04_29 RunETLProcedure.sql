DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `RunETLProcess`(IN `pSeasonID` INT, IN `pImportedFileID` INT)
BEGIN
...
ALTER TABLE dw_rpt06_staging DISABLE KEYS;    
...
CALL LogTableOperation(pImportedFileID, (SELECT id FROM processed_table WHERE table_name = 'dw_rpt06_staging'), 1, ROW_COUNT());

ALTER TABLE dw_rpt06_staging ENABLE KEYS;    
